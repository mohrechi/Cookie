...
void input()
{
    fscanf(fin,"%d",&offerNum);
    for(int i=0;i<offerNum;i++)
    {
        fscanf(fin,"%d",&offer[i].n);
        for(int j=0;j<offer[i].n;j++)
        {
            fscanf(fin,"%d%d",&offer[i].code[j],&offer[i].num[j]);
        }
        fscanf(fin,"%d",&offer[i].price);
    }
    fscanf(fin,"%d",&needsNum);
    for(int i=0;i<needsNum;i++)
    {
        fscanf(fin,"%d%d%d",&needs[i].code,&needs[i].num,&needs[i].price);
    }
}
void code()
{
